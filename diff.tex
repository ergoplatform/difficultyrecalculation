...
During the epoch B the adversary mines with difficulty $D_1 = R_0 \cdot \texp$ is calculated from honest miners $R_0$ only.
...
If $\reward$ is block reward and $C$ is hash calculation cost, the honest miner profit is
\begin{equation}
{({M \cdot R_a \over R_0 + R_a})\cdot \reward - M \cdot R_a\cdot T\cdot C=M\cdot R_a\cdot ({\reward \over R_0+R_a}-T \cdot C)}
\end{equation}
per epoch.
At the same time adversarial profit is
\begin{equation}
{{M\cdot R_a \over 2\cdot (R_0+R_a)}\cdot (\reward - T\cdot R_0\cdot C)}
\end{equation}
...
[TODO: ???]
\begin{equation}
{{\reward \over C} < T\cdot (R_0+2\cdot R_a)}.
\end{equation}
