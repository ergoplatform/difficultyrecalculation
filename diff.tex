...
Most of difficulty retargeting algorithms assume total computational power involved in mining process does not significantly change from epoch to epoch. In contrast, real networks show that significant variance in computational power could happen for long periods.
For example, we show in this paper that continuous growth of computational power in Bitcoin network leads to decreasing mean delay between blocks by 7\% from an expected value.
...
On the other side, target recalculation algorithm should be simple enough. It is preferable to use integer arithmetic in the algorithm since all the nodes in the peer-to-peer network, implemented in different programming languages and running on different platforms, have to agree absolutely on the calculated difficulty.

Original Bitcoin whitepaper states that the security of the system is guaranteed as long as there is no attacker in possession of half or more of the total computational power used to maintain the system~\cite{Nakamoto2008}.
