...
For Bitcoin observed time interval $\approx$\textit{9 min 20 sec} is less than planned value $\texp =10 min$ due to continuous growth of network computational power.
...
Consider the network has hash rate $R_0$ provided by honest miners and the adversary has additional hash rate $R_a=R_0\cdot p, 0 < p < 1 $ to turn it on or off.
Before epoch A the adversary mines all the time with difficulty \(D_0 = (R_0+R_a) \cdot T\) and will mine $M \cdot R_a \over R_0 + R_a$ blocks per epoch (of $M$ blocks) in average by spending $M \cdot R_a \cdot T$ computational power for them.
During the epoch B the adversary mines with difficulty $D_1 = R_0 \cdot T$ is calculated from honest miners $R_0$ only.
He will mine \(M\cdot R_a\over 2(R_0+R_a)\) blocks per epoch in average spending $M\cdot T\cdot R_a\cdot R_0\over 2 \cdot (R_0+R_a)$ computational power for them.
Consequently honest miner will spend \((R_0+R_a)T\) computational power per block, whereas attacker will just spend $R_0 \cdot T$ computational power per block.
...
\begin{equation}
{({M\cdot R_a\over R_0+R_a})\cdot B-M\cdot R_a\cdot T\cdot C=M\cdot R_a\cdot ({B\over R_0+R_a}-T \cdot C)}
\end{equation}
per epoch.
