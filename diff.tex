...
The Bitcoin white-paper, states that the security of the system is guaranteed as long as honest miners control more then half of the total computational power used to maintain the system.
In this paper we analyze the new way of attack on the blockchain, based on manipulating mining difficulty.
In spite of the fact that it don't allow to break blockchain security by double-spending, the attacker receive significant profit from this attack and average time interval between blocks increases as a side-effect.
To reduce the attacker profit and to improve the stability of block times, we proposed an alternative difficulty control, that performs much better both in terms of block rate and in terms of attacker profit.
