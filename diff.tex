...
In this paper we introduce and analyze a new kind of attack on the based on mining difficulty manipulations. A malicious miner is increasing his mining profits from the attack. An average time interval between blocks increases as a side-effect.
...
Each hash attempt yields a valid block with a probability proportional to \(1 \over D\) and block frequency is proportional to \(R \over D\) where \(R\) is effective network hash rate~(total number of calls to the hash function across the network).
